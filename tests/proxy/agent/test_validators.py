...
def test_validator_browser_actions(validator):
    ...
    valid, msg = validator._validate_tool_args("browser_action", {"action": "goto", "url": ""})
    assert not valid
    
    # Valid navigation
    valid, msg = validator._validate_tool_args("browser_action", {"action": "goto", "url": "http://1.com"})
    assert valid


def test_validator_read_file(validator):
    # Negative Offset
    valid, msg = validator._validate_tool_args("read_file", {"path": "test.txt", "offset": -5})
    assert not valid
    
    # Good constraints
    valid, msg = validator._validate_tool_args("read_file", {"path": "test.txt", "offset": 10, "limit": 100})
    assert valid
...
def test_validator_report_rejections(validator):
    ...
    valid, msg = validator._validate_tool_args("create_vulnerability_report", args.copy())
    assert not valid
    assert "too short" in msg

    # Needs HTTP Evidence on non-CTF
    args_http = args.copy()
    args_http["poc_script_code"] = "curl -X GET http://example.com/api/test" + ("." * 50)
    ...
    args_url = args.copy()
    args_url["poc_description"] = "We sent the payload and observed HTTP 200" + ("." * 80)
    args_url["poc_script_code"] = "python3 exploit.py" + ("." * 50) # No HTTP or curl present
    ...
    args_valid["poc_script_code"] = "curl -X GET http://example.com/api/test" + ("." * 50)
    args_valid["poc_description"] = "We sent the payload and observed HTTP 200 containing user data." + ("." * 80)
    ...
    valid, msg = validator._validate_tool_args("create_vulnerability_report", args_valid)
    if not valid:
