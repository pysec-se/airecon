...
class TestHTTPEvidenceValidationGap1:
    ...
    def test_reject_http_status_alone(self):
        ...
        result = self.validator._validate_tool_args(
    ...
    def test_reject_redirect_alone(self):
        ...
        is_valid, msg = result
        assert not is_valid, (
            "Should reject status change without explaining what access was gained"
        )
        assert msg is not None and "impact not documented" in msg.lower()
    ...
    def test_runtime_replay_mismatch_rejected_in_strict_phase(self):
        ...
        self.validator._get_current_phase = lambda: type(
            "P", (), {"value": "EXPLOIT"}
        )()  # type: ignore[attr-defined]
        self.validator.state = type(
            "S",
            (),
            {
                "tool_history": [
        ...
        is_valid, msg = self.validator._validate_tool_args(
            "create_vulnerability_report",
            {
                "poc_script_code": (
                    "#!/bin/bash\n"
