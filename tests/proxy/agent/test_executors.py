...
async def test_execute_local_browser_tool(agent, mocker):
    mocker.patch('airecon.proxy.agent.executors.browser_action', return_value={"title": "Test Page"})
...
async def test_execute_filesystem_tool_read(agent, mocker):
    mocker.patch('airecon.proxy.agent.executors.read_file', return_value={"success": True, "result": "file content"})
...
async def test_execute_report_tool(agent, mocker):
    mocker.patch('airecon.proxy.agent.executors.create_vulnerability_report', return_value={"success": True, "finding_id": "VULN-1"})
