...
async def test_agent_initialization(agent_loop, mocker):
    mocker.patch('airecon.proxy.system.get_system_prompt', return_value="You are AIRecon.")
...
async def test_agent_duplicate_command(agent_loop):
    ...
    is_dup, msg = agent_loop._is_duplicate_command("create_file", {"path": "test", "content": "1"})
    assert not is_dup
